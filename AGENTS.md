...
- `just release <version>`: cut a tagged release (e.g. `just release 1.2.3`). Builds the binary with `Build`/`Version` ldflags, packages `dist/rift-<version>-<os>-<arch>.tar.gz`, then runs `gh release create`/`gh release upload`. Requires a clean working tree on `main`; `os`/`arch` default to `darwin`/`amd64` in the `justfile`.
